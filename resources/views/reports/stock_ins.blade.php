...
@section('page_desc', 'Kelola laporan stok masuk barang inventaris.')
...
<div class="page-padding space-y-4">
    ...
    {{-- FILTER TANGGAL --}}
    <div class="card p-4">
        <form method="GET" action="{{ route('reports.stockIns') }}"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('reports.stockIns') }}" class="btn-secondary w-full md:w-auto text-center">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="table-wrapper">
        <table class="table">
            ...
            <tbody>
                @forelse($stockIns as $i)
                @php
                    $buy = $i->buy_price ?? $i->product->buy_price;
                    $subtotal = $buy * $i->qty;
                @endphp
                <tr>
                    ...
                    <td class="text-right">Rp {{ number_format($buy, 0, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($subtotal, 0, ',', '.') }}</td>
                    <td class="text-slate-500">{{ $i->note ?? '-' }}</td>
                </tr>
                @empty
                <tr>
    ...
    <div class="flex justify-end gap-2">
        <a href="{{ route('reports.stockIns.pdf', request()->query()) }}" class="btn-secondary">
            Export PDF
        </a>
        <a href="{{ route('reports.stockIns.excel', request()->query()) }}" class="btn-primary">
            Export Excel
        </a>
    </div>
